...
PBL-Schulaufgabe von **Gruppe 10**: Lösungskonzept und vereinfachter technischer Durchstich für ein Apache-Kafka-basiertes ESP/CEP-Szenario (Erkennung verspäteter Aussendienstfahrer und Dashboard-Meldung). Dozent: Marc Schaaf.

Ziel laut Aufgabenstellung: **zeitnahe Verarbeitung** (nicht dauerhafte Speicherung) der Position-Events, zwei-stufiger CEP-Flow.

## Stack & Struktur

- **Sprache/Build:** Java + Maven, Kafka Streams `3.5.0` (siehe `Materialien zu Umsetzung/kafka_sample/pom.xml`)
- **Startpunkt für eigenen Code:** [`Materialien zu Umsetzung/kafka_sample/`](Materialien%20zu%20Umsetzung/kafka_sample/) — alle Dependencies konfiguriert. Helper in `Utils.java` benutzen (Regex-Parsing, REST-Aufruf), nicht selbst neu schreiben.
- **Group-Prefix für eigene Topics:** `group10-…` (z.B. `group10-route-timing`). Die im Step-by-Step-Guide gezeigten `group1234-…` sind Platzhalter.
...
- **Build/Run:** `mvn -DskipTests package` im `kafka_sample`-Verzeichnis; Run via IDE oder `mvn exec:java -Dexec.mainClass=Launcher`.
...
| [`Materialien zu Umsetzung/kafka_sample/src/main/java/Utils.java`](Materialien%20zu%20Umsetzung/kafka_sample/src/main/java/Utils.java) | Wann immer GPS-Strings, REST-Calls oder Delay-Strings geparst/erzeugt werden — Helper sind hier |
| [`Materialien zu Umsetzung/kafka_sample/src/main/java/Launcher.java`](Materialien%20zu%20Umsetzung/kafka_sample/src/main/java/Launcher.java) | Beim Aufsetzen einer neuen Streams-App — Boilerplate (Config, Topology, Shutdown-Hook) |
...
- **`delays`-Topic wird von allen Gruppen geteilt** — das Dashboard differenziert nicht. Eigene Zwischen-Topics zwingend mit `group10-` präfixieren.
